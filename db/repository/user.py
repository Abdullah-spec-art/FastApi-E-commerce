...
def create_user(db: Session, user: UserCreate): 
        ...
        if user.role not in ["buyer", "seller"]:
            raise HTTPException(status_code=400, detail="Invalid role. Choose either 'buyer' or 'seller'.")

        hashed_password = hash_password(user.password)
        otp=generate_otp()
        email_verification=False
        db_user = User(username=user.name, email=user.email, password=hashed_password,role=user.role ,otp=otp,email_verification=email_verification)
        db.add(db_user)
        db.commit()
        db.refresh(db_user)
        ...
        data=Data(
            name=user.name,
            email=user.email,
            email_verified=db_user.email_verification
        )
...
def login_user(db: Session, user_login: UserLogin):
    ...
    if user.email_verification is False:
        ...
        user.otp_created_at=otp_created
        user.otp=otp
        db.add(user)
        db.commit()
        db.refresh(user)
        ...
        return Response[Data](data=data,message="Email not verified. The new OTP has been to your email")
...
def verify_otp(db: Session, user:OTPVerification):
    ...
    otp_expires_time = db_user.otp_created_at + timedelta(minutes=5)
    ...
    return Response[Data](data=data,message= "OTP verification completed successfully.")
...
def reset_password(db:Session, user:UserEmailSchema):
    ...
    db_user.otp=otp
    db_user.otp_created_at=otp_created
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
